		...
		init: function(table, thisWidget, c, wo){
			...
			c.$tbodies
				...
				.on('blur focusout keydown '.split(' ').join('.tseditable '), '[contenteditable]', function(e){
					...
					if ( t && $this.data('before') !== $this.val() ) {
						valid = $.isFunction(wo.editable_validate) ? wo.editable_validate( $this.val(), $this.data('original'), $this ) : $this.html();//added 3rd arg
						//to give validation method a reference to the cell we are editing
